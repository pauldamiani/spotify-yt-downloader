...
from converter import convert_songs
...
def ask_conversion_folder():
    options = {
        'type': 'list',
        'name': 'choice',
        'message': 'Where are the songs to be converted',
        'choices': [
            '1.Current folder',
    ...
    if '1' in choice:
        ...
        ques = {
            'type': 'input',
            'name': 'path',
            'message': 'Enter path where songs are:'
        }
...
def youtubedl():
    ...
    if '1' in choice:
        ...
        data = ask_download_particular_song()
        song_string = data['artist'] + " " + data['song']
        songs = [yt.get_song(song_string)] # List of 1 song
